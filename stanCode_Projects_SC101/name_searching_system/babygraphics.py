...
def draw_fixed_lines(canvas):
    ...
    for i in range(len(YEARS)):
        ...
        canvas.create_text(x+TEXT_DX, CANVAS_HEIGHT - GRAPH_MARGIN_SIZE, anchor=tkinter.NW, text=YEARS[i])
...
def draw_names(canvas, name_data, lookup_names):
    ...
    for name in lookup_names:
        ...
        for year in YEARS:
            ...
            x = get_x_coordinate(CANVAS_WIDTH, i)
            if rank is not None:
                y = GRAPH_MARGIN_SIZE + int(rank) * ((CANVAS_HEIGHT - 2 * GRAPH_MARGIN_SIZE) / MAX_RANK)
            else:
                rank = '*'
                y = CANVAS_HEIGHT-GRAPH_MARGIN_SIZE

            color_index = lookup_names.index(name) % len(COLORS)
            canvas.create_text(x+TEXT_DX, y, text=name+" "+rank, anchor=tkinter.SW, fill=COLORS[color_index])
            if i != 0:
