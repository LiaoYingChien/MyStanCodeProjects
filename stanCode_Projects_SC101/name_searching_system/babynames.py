...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name in name_data:
        if year in name_data[name]:
            high_rank = min(int(name_data[name][year]), int(rank))
            name_data[name][year] = str(high_rank)
        else:
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        # read first line and skip '/n'
        year = f.readline().rstrip()

        # read data and skip first line
        lines = f.readlines()
        for line in lines:
            data = line.split(',')
            rank = data[0].strip()
